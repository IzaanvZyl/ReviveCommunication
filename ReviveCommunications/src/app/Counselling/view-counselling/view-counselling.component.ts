...
@Component({
  selector: 'app-view-counselling',
  templateUrl: './view-counselling.component.html',
  styleUrls: ['./view-counselling.component.scss']
})
export class ViewCounsellingComponent implements OnInit {
  ...
  ngOnInit(){
    this.CounsellingView = this.service.getCounselling().subscribe(x => {
      console.log(x);
      this.CounsellingView = x
    });
  ...
  onRemove(Counselling)
  {
    if(confirm('Are you sure you want to Resolve this Request?'))
    {
      this.service.ResolveCounselling(Counselling).subscribe(res=>
        {
          this.service.getCounselling();
