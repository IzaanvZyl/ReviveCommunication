...
export class MaintainOrgIndivPosComponent implements OnInit {
  ...
  onSubmit(oip){
    ...
    if(confirm('Are you sure you want to update this Organisational Individual Position?')){
      this.OipService.updateOrgIndivPos(oip).subscribe(x =>{
  ...
  presentToast(position, status) {
    ...
    this.toastrService.show(
      status || 'Success',
      `Organisational Individual Position updated Successfully`,
      { position, status});
  }
