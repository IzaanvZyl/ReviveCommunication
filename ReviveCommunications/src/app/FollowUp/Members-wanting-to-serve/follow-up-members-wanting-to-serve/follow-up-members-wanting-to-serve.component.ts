...
export class FollowUpMembersWantingToServeComponent implements OnInit {
  ...
  Person:any;
	  session:any;
  constructor(private sidebarService: NbSidebarService, private SalvationService: SalvationServiceService,  private toastrService: NbToastrService, private router: Router, private loginService: LoginService) { }
...
MemberServe;
  ...
  if(!localStorage.getItem("accessToken")){
    ...
    this.SalvationService.getMembersServe().subscribe(data=> {this.MemberServe = data;
     });
     console.log(this.MemberServe);
  }

  FollowedUp(MemberID){
    this.SalvationService.FollowUpCompletedMember(MemberID);
  ...
  NoAnswer(MemberID)
  {
    this.SalvationService.FollowUpNoAnswerMember(MemberID);
  ...
  NoAnswerMessage(position, status) {
    ...
    this.toastrService.show(
      status || 'Success',
      `No Answer. Follow-up unsuccessful.`,
      { position, status});
  }
